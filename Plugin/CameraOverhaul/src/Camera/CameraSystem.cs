...
internal sealed class CameraSystem
{
    ...
    private void InsanityOffset(in CameraContext context, double dt, ConfigData cfg)
    {
        ...
        if (threshold >= 1.0 || context.insanity < threshold) return;

        float panic = (float)MathUtils.Clamp01((context.insanity - threshold) / (1.0 - threshold));
        if (panic <= 0f) return;
    ...
    {
        double cueMultiplier = context.leviathanCue switch
        {
            LeviathanCue.Emerging => cfg.general.leviathanWarningTremorMultiplier,
            LeviathanCue.Growl => cfg.general.leviathanGrowlShakeMultiplier,
            LeviathanCue.Rumble => cfg.general.leviathanRumbleShakeMultiplier,
            _ => 1.0,
        };
        double target = cfg.general.leviathanProximityStrength * context.leviathanProximity01 * cueMultiplier;
    ...
    private void FreezeOffset(in CameraContext context, double dt, ConfigData cfg)
    {
        double target = context.inSnow ? 1.0
            : context.shipWithDoorsOpen ? cfg.general.freezeShipOpenDoorTarget
            : 0.0;
        ...
        double severityCurve = _freezeDuration * _freezeDuration * (3.0 - (2.0 * _freezeDuration));

        _freezeTime += dt * FREEZING_SPEED;
        double intensity = cfg.general.freezeStrength * severityCurve;
        _impairOffset.x += (float)(Noise.Sample(FreezeNoise, _freezeTime, 16100.0) * intensity);
        _impairOffset.y += (float)(Noise.Sample(FreezeNoise, _freezeTime * 1.1, 16200.0) * intensity * 0.6);
