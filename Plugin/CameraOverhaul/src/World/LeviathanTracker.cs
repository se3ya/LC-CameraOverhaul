...
internal static class LeviathanTracker
{
    ...
    private const float AudibleFadeBand = 0.25f;
    ...
    internal static float GetLocalProximity01(PlayerControllerB player, out LeviathanCue cue)
    {
        ...
        bool audible = sfx != null && sfx.isPlaying && nearestDist < Mathf.Min(sfx.maxDistance, radius);
        if (!audible) return 0f;

        cue = nearest.inEmergingState ? LeviathanCue.Emerging : ClassifyClip(sfx!.clip);
        float linear = 1f - Mathf.Clamp01(nearestDist / radius);
        float edge = Mathf.Clamp01((radius - nearestDist) / (radius * AudibleFadeBand));
        return Mathf.Max(linear, AudibleProximityFloor * edge);
    }
